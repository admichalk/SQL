-- 1. Identify the number of employees within each department that share the same birth month. 
...
-- 2. Find the 3-month rolling average of total revenue from purchases given a table with users, their purchase amount, and date purchased. 
...
-- 3. Find the number of days a US track has stayed in the 1st position for both the US and worldwide rankings on the same day. 
...
-- 4. Find the number of employees who received the bonus and who didn't. 
...
-- 5. Find the top 5 highest paid and top 5 least paid employees in 2012. 
...
-- 7. Write a query to find the number of days between the longest and least tenured employee still working for the company. 
...
-- 8.Which company had the biggest month call decline from March to April 2020? 
...
-- 10.The company you work for has asked you to look into the average order value per hour during rush hours in the San Jose area. 
...
-- 11.Write a query to calculate the longest period (in days) that the company has gone without hiring anyone. 
...
-- 14. A group of travelers embark on world tours starting with their home cities. 
...
-- 15.Write a query to get the list of managers whose salary is less than twice the average salary of employees reporting to them. 
...
-- 19.After a new user creates an account and starts watching videos, the user ID, video ID, and date watched are captured in the database. 
...
-- 20. Christmas is quickly approaching, and your team anticipates an increase in sales. 
...
-- 21. Tiktok want to find out what were the top two most active user days during an advertising campaign they ran in the first week of 
-- August 2022 (between the 1st to the 7th). Identify the two days with the highest user activity during the advertising campaign. 
-- They've also specified that user activity must be measured in terms of unique users.
-- Output the day, date, and number of users. Be careful that some function can add a padding (whitespaces) around the string, 
...
-- 23. It's time to find out who is the top employee. 
...
-- 27. Find the average number of days between the booking and check-in dates for AirBnB hosts. 
...
-- 28. Find the first and last inspections for vermin infestations per municipality. 
...
-- 29. Find top crime categories in 2014 based on the number of occurrences. 
...
-- 31. You have been asked to find the employee with the highest salary in each department. 
...
-- 33. Find the number of athletes who participated in the Olympics that hosted in European cities. 
...
-- 35. Find the employee who earned the lowest total payment with benefits from a list of employees 
-- who earned more from other payments compared to their base pay. 
...
-- 36. Find all combinations of 3 numbers that sum up to 8. 
...
-- 38. Find all provinces which produced more wines in 'winemag_p1' than they did in 'winemag_p2'. 
-- Output the province and the corresponding wine count. Order records by the wine count in descending order.
...
-- 39. Find the vintage years of all wines from the country of Macedonia. The year can be found in the 'title' column. 
...
-- 41. Find matching pairs of Meta/Facebook employees such that they are both of the same nation, different age, 
-- same gender, and at different seniority levels. Output ids of paired employees.
SELECT a.id, b.id
...
-- 42. Find how many events happened on MacBook-Pro per company in Argentina from users that do not speak Spanish. 
...
-- Your output should include the employee's ID, salary, and department, as well as a column salary_type that 
-- categorizes the output by: * 'Highest Salary' represents the highest salary * 'Lowest Salary' represents the lowest salary
...
-- 44. Rank each host based on the number of beds they have listed. 
...
-- 45. Meta/Facebook Messenger stores the number of messages between users in a table named 'fb_messages'. 
